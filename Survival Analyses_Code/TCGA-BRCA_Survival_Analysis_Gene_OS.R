## ==================================================
## TCGA-BRCA OS (all patients): Gene-based Cox + LRT (with expr:CNA interaction + filtered sheets)
## ==================================================
...
shorten_model_name <- function(model_name) {
  # If this is a a filtered sheet name, keep the suffix as is
...
## --------------------------------------------------
## 3) TCGA-BRCA OS (all patients): survival + covariates (Age, Stage_simple, PAM50)
## --------------------------------------------------
...
genes_tcga <- intersect(
  genes,
  intersect(
    intersect(tcga_expr$Gene_Symbol, tcga_cna_binary$Gene_Symbol),
    fisher_by_gene$Gene_Symbol
  )
)

message("Number of genes used for TCGA-BRCA OS gene-based Cox models: ",
        length(genes_tcga))

## --------------------------------------------------
## 6) TCGA gene-level expression & CNA (long format, aligned to survival)
## --------------------------------------------------

expr_long_tcga <- tcga_expr %>%
  filter(Gene_Symbol %in% genes_tcga) %>%
  pivot_longer(
    cols      = -Gene_Symbol,
...
cna_long_tcga_aligned <- cna_long_tcga %>%
...
fit_one_gene_one_model_tcga <- function(gene_name, covars, model_name) {
  d <- expr_long_tcga %>%
    filter(Gene_Symbol == gene_name) %>%
    dplyr::rename(expr = expr) %>%
    inner_join(
      cna_long_tcga_aligned %>%
        filter(Gene_Symbol == gene_name) %>%
...
## --------------------------------------------------
## 9) Fit all models for all genes (TCGA-BRCA OS, all)
## --------------------------------------------------
...
for (i in seq_len(nrow(model_specs))) {
  ...
  model_rows <- map(
    genes_tcga,
    ~ fit_one_gene_one_model_tcga(.x, covars, model_name)
  )
  ...
  if (nrow(model_df) > 0) {
    ...
    message("!!! TCGA-BRCA OS (all) | No suitable gene for model: ", model_name)
  }
}
...
get_gene_data_tcga <- function(gene_name) {
  expr_long_tcga %>%
    filter(Gene_Symbol == gene_name) %>%
    dplyr::rename(expr = expr) %>%
    inner_join(
...
fit_pair_lrt_gene_tcga <- function(gene_name, d, vars_small, vars_big,
                                   name_small, name_big) {
...
lr_tests_for_gene_tcga <- function(gene_name) {
  d <- get_gene_data_tcga(gene_name)
  n_events <- sum(d$OS_EVENT, na.rm = TRUE)
...
## --------------------------------------------------
## 11) Collect LRT results for all genes (TCGA-BRCA OS all)
## --------------------------------------------------
...
total_genes_tcga <- length(genes_tcga)
...
message(
  "All gene-level Cox models for TCGA-BRCA (OS, all patients) with BOTH expr+cna and expr+cna+expr:CNA models, ",
