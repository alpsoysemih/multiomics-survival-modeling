## ==================================================
## TCGA-BRCA OS (all patients): Pathway-based Cox + LRT
## ==================================================
...
## Shorten model names (expr→E, cna→C, exprCNA→EC, Stage→Stg)
...
## --------------------------------------------------
## 2) TCGA-BRCA data (expression, CNA, survival)
## --------------------------------------------------
...
## --------------------------------------------------
## 3) TCGA-BRCA OS (ALL patients): survival + covariates
## --------------------------------------------------
...
message("Number of ALL TCGA-BRCA samples with OS data: ", nrow(tcga_surv_small))
...
## --------------------------------------------------
## 6) TCGA expression & CNA in long format
## --------------------------------------------------
...
global_expr_score_tcga <- path_expr_score_tcga_sample %>%
  group_by(pathway) %>%
  summarise(
...
global_cna_score_tcga <- path_cna_score_tcga_sample %>%
  group_by(pathway) %>%
  summarise(
...
pathway_scores_tcga <- global_expr_score_tcga %>%
  full_join(global_cna_score_tcga, by = "pathway")

num_cols_ps_tcga <- sapply(pathway_scores_tcga, is.numeric)
pathway_scores_tcga[, num_cols_ps_tcga] <- lapply(
  pathway_scores_tcga[, num_cols_ps_tcga],
...
## --------------------------------------------------
## 11) Cox function for a single pathway + single model (TCGA)
## --------------------------------------------------

fit_one_pathway_one_model_tcga <- function(pathway_name, covars, model_name) {
  d <- path_expr_score_tcga_sample %>%
    filter(pathway == pathway_name) %>%
    inner_join(
      path_cna_score_tcga_sample %>% filter(pathway == pathway_name),
      by = c("sample", "pathway")
    ) %>%
    inner_join(tcga_surv_small, by = "sample") %>%
    transmute(
      pathway      = pathway,
      sample       = sample,
...
## --------------------------------------------------
## 12) Fit all models for all pathways (TCGA-BRCA OS, all)
## --------------------------------------------------

pathways_tcga <- intersect(
  path_expr_score_tcga_sample$pathway,
  path_cna_score_tcga_sample$pathway
)

results_by_model_tcga <- list()

for (i in seq_len(nrow(model_specs))) {
  model_name <- model_specs$model[i]
  covars     <- model_specs$covars[[i]]
  
  message("=== TCGA-BRCA OS (all) | Fitting model: ", model_name,
          " | covariates: ", paste(covars, collapse = ", "), " ===")
  
  model_rows <- map(
    pathways_tcga,
    ~ fit_one_pathway_one_model_tcga(.x, covars, model_name)
  )
  ...
  if (nrow(model_df) > 0) {
    results_by_model_tcga[[model_name]] <- model_df
    message(">>> TCGA-BRCA OS (all) | Finished model: ", model_name,
            " | number of fitted pathways: ",
            length(model_rows) - sum(sapply(model_rows, is.null)))
  } else {
    message("!!! TCGA-BRCA OS (all) | No suitable pathway for model: ", model_name)
  }
}
...
get_pathway_data_tcga <- function(pathway_name) {
...
fit_pair_lrt_pathway_tcga <- function(pathway_name, d, vars_small, vars_big,
                                      name_small, name_big) {
  ## "Stage" → "Stage_simple"; exprCNA is only a flag
...
lr_tests_for_pathway_tcga <- function(pathway_name) {
  d <- get_pathway_data_tcga(pathway_name)
  n_events <- sum(d$OS_EVENT, na.rm = TRUE)
  ...
  for (i in seq_along(model_names)) {
    for (j in seq_along(model_names)) {
      ...
      ## 1) All comparisons involving expr or exprCNA → LR_expr
      if (has_expr || has_exprCNA) {
        lr_expr_list[[length(lr_expr_list) + 1L]] <- row
      }
      ## 2) All comparisons involving cna or exprCNA → LR_cna
      if (has_cna || has_exprCNA) {
        lr_cna_list[[length(lr_cna_list) + 1L]]  <- row
      }
      ## 3) Baseline: Age + Stage + PAM50 as the smaller model
...
## --------------------------------------------------
## 14) Collect LRT results for all pathways (TCGA-BRCA OS all)
## --------------------------------------------------

lr_expr_list_all_tcga <- list()
lr_cna_list_all_tcga  <- list()
lr_base_list_all_tcga <- list()

for (pw in pathways_tcga) {
  lr_res <- lr_tests_for_pathway_tcga(pw)
  if (is.null(lr_res)) next
  
  if (!is.null(lr_res$expr) && nrow(lr_res$expr) > 0) {
    lr_expr_list_all_tcga[[pw]] <- lr_res$expr
  }
  if (!is.null(lr_res$cna)  && nrow(lr_res$cna)  > 0) {
    lr_cna_list_all_tcga[[pw]]  <- lr_res$cna
  }
  if (!is.null(lr_res$base) && nrow(lr_res$base) > 0) {
    lr_base_list_all_tcga[[pw]] <- lr_res$base
  }
}

lr_expr_df_tcga <- lr_expr_list_all_tcga %>%
  compact() %>%
  bind_rows() %>%
  distinct(pathway, small_model, big_model, .keep_all = TRUE)

lr_cna_df_tcga <- lr_cna_list_all_tcga %>%
  compact() %>%
  bind_rows() %>%
  distinct(pathway, small_model, big_model, .keep_all = TRUE)

lr_base_df_tcga <- lr_base_list_all_tcga %>%
  compact() %>%
  bind_rows() %>%
  distinct(pathway, small_model, big_model, .keep_all = TRUE)
...
## Lower bound for BH: number of unique genes in the enrichment
total_genes_tcga <- length(unique(path_genes_long$Gene_Symbol))
...
process_model_df <- function(df) {
  ...
  ## BH-adjustment for all p-columns
  ...
  ## Sort rows: if CI_lo_cna exists, sort by it (desc), otherwise sort by CI_lo_expr (desc)
...
results_by_model_tcga_proc <- lapply(results_by_model_tcga, process_model_df)

## Filtered sheets:
##  - CI_lo_expr > 1 OR CI_lo_cna > 1 OR CI_lo_expr_cna > 1
##  - no filtered sheet for the baseline (Age_Stage_PAM50) model
filtered_by_model_tcga <- list()

for (nm in names(results_by_model_tcga_proc)) {
  df <- results_by_model_tcga_proc[[nm]]
  ...
  filtered_by_model_tcga[[paste0(nm, "_filtered")]] <- df_filt
}
...
lr_expr_df_tcga <- process_lrt_df(lr_expr_df_tcga)
lr_cna_df_tcga  <- process_lrt_df(lr_cna_df_tcga)
lr_base_df_tcga <- process_lrt_df(lr_base_df_tcga)

## Also shorten model names in LRT tables (E / C / EC / Age / Stg / PAM50)
if (nrow(lr_expr_df_tcga) > 0) {
  lr_expr_df_tcga <- lr_expr_df_tcga %>%
    mutate(
...
if (nrow(lr_cna_df_tcga) > 0) {
  lr_cna_df_tcga <- lr_cna_df_tcga %>%
    mutate(
      small_model = vapply(small_model, shorten_model_name, character(1)),
      big_model   = vapply(big_model,   shorten_model_name, character(1))
    )
...
if (nrow(lr_base_df_tcga) > 0) {
  lr_base_df_tcga <- lr_base_df_tcga %>%
    mutate(
      small_model = vapply(small_model, shorten_model_name, character(1)),
      big_model   = vapply(big_model,   shorten_model_name, character(1))
    )
...
## 16) Excel export:
##   - Pathway_scores
##   - main models (with shortened names)
##   - _filter (CI_lo_expr / CI_lo_cna / CI_lo_expr_cna > 1)
##   - extra _filter2 for models with exprCNA (only CI_lo_expr_cna > 1)
##   - LRT sheets
## --------------------------------------------------

xls_models_tcga <- list()

for (nm in names(results_by_model_tcga_proc)) {
  df <- results_by_model_tcga_proc[[nm]]
  
  nm_short <- shorten_model_name(nm)  ## E_C_EC_Age_Stg_PAM50 etc.
  
  ## main sheet
  xls_models_tcga[[nm_short]] <- df
  
  if (nm != "Age_Stage_PAM50") {
    ## _filter sheet
    filt_name <- paste0(nm, "_filtered")
    df_filt   <- filtered_by_model_tcga[[filt_name]]
    
    if (!is.null(df_filt)) {
      nm_filt_short <- paste0(nm_short, "_filter")
      xls_models_tcga[[nm_filt_short]] <- df_filt
    }
    
    ## For interaction models, filter2: only CI_lo_expr_cna > 1
    if ("CI_lo_expr_cna" %in% names(df)) {
      df_filt2 <- df %>%
      ...
      xls_models_tcga[[nm_filt2_short]] <- df_filt2
    }
  }
}

out_file_tcga <- file.path(
  tcga_results_dir,
...
write_xlsx(
  c(
    list(Pathway_scores = pathway_scores_tcga),
    xls_models_tcga,
    list(
      LR_expr     = lr_expr_df_tcga,
      LR_cna      = lr_cna_df_tcga,
      LR_baseline = lr_base_df_tcga
    )
  ),
  out_file_tcga
)
...
message(
  "All pathway-level Cox models for TCGA-BRCA (OS, all patients) with BOTH expr+cna and expr+cna+expr:CNA models, ",
