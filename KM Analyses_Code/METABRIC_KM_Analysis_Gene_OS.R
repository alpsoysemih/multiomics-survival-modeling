...
## Output folder for KM plots (OS, all METABRIC)
km_dir <- file.path(metabric_results_dir, "METABRIC_KM_Gene_OS")
...
## --------------------------------------------------
## 4) Expression & CNA in long format
## --------------------------------------------------
...
## --------------------------------------------------
## 5) Helper: get per-gene survival + omics data (OS, ALL patients)
## --------------------------------------------------

get_gene_surv_data <- function(gene_name) {
  expr_long_metabric %>%
...
plot_km_cna <- function(gene_name,
  ...
  if (length(unique(d$cna_group)) < 2) {
    message("CNA KM (OS all): only one CNA group present, skipping gene: ", gene_name)
    return(NULL)
  }
  ...
  if (require_sig_p && (is.na(lr_p) || lr_p >= 0.05)) {
    message("CNA KM (OS all): log-rank p >= 0.05, skipping gene: ", gene_name)
    return(NULL)
  }
  ...
  p <- ggsurvplot(
    ...
    title                = paste0(gene_name, " - CNA (OS, METABRIC)"),
    xlab                 = "Time (months)",
    ylab                 = "Probability",
    ggtheme              = theme_bw(),
  ...
  message("Saved METABRIC OS CNA KM plot: ", out_file)
  invisible(out_file)
}

## 6b) Expression – median cut (2 groups)
plot_km_expr_median <- function(gene_name,
                                HR, LCL, UCL, PVAL,
                                out_dir,
                                model_suffix,
                                require_sig_p = TRUE) {
  ...
  if (nrow(d) < 30 || sum(d$OS_EVENT, na.rm = TRUE) < 5) {
    message("Expr-median KM (OS all): insufficient data (n<30 or <5 events), skipping gene: ", gene_name)
    return(NULL)
  }
  ...
  if (length(unique(d$expr_group)) < 2) {
    message("Expr-median KM (OS all): only one expression group present, skipping gene: ", gene_name)
    return(NULL)
  }
  ...
  if (require_sig_p && (is.na(lr_p) || lr_p >= 0.05)) {
    message("Expr-median KM (OS all): log-rank p >= 0.05, skipping gene: ", gene_name)
    return(NULL)
  }
  ...
  p <- ggsurvplot(
    ...
    title                = paste0(gene_name, " - Expression median (OS, METABRIC)"),
    xlab                 = "Time (months)",
    ylab                 = "Probability",
    ggtheme              = theme_bw(),
    tables.theme         = theme_bw()
  ...
  message("Saved METABRIC OS expression-median KM plot: ", out_file)
  invisible(out_file)
}

## 6c) Expression – optimal cut (maxstat, 2 groups)
plot_km_expr_optimal <- function(gene_name,
                                 HR, LCL, UCL, PVAL,
                                 out_dir,
                                 model_suffix,
                                 require_sig_p = TRUE) {
  ...
  if (nrow(d) < 30 || sum(d$OS_EVENT, na.rm = TRUE) < 5) {
    message("Expr-optimal KM (OS all): insufficient data (n<30 or <5 events), skipping gene: ", gene_name)
    return(NULL)
  }
  ...
  cut_res <- tryCatch(
    ...
    error = function(e) {
      message("Expr-optimal KM (OS all): surv_cutpoint failed for ", gene_name, " – skipping. Error: ", e$message)
      return(NULL)
    }
  )
  ...
  if (length(unique(d$expr_group_opt)) < 2) {
    message("Expr-optimal KM (OS all): only one optimal expression group present, skipping gene: ", gene_name)
    return(NULL)
  }
  ...
  if (require_sig_p && (is.na(lr_p) || lr_p >= 0.05)) {
    message("Expr-optimal KM (OS all): log-rank p >= 0.05, skipping gene: ", gene_name)
    return(NULL)
  }
  ...
  p <- suppressWarnings(
    ggsurvplot(
      ...
      title                = paste0(gene_name, " - Expression optimal (OS, METABRIC)"),
      xlab                 = "Time (months)",
      ylab                 = "Probability",
      ggtheme              = theme_bw(),
      tables.theme         = theme_bw()
  ...
  message("Saved METABRIC OS expression-optimal KM plot: ", out_file)
  invisible(out_file)
}
...
plot_km_expr_cna_interaction <- function(gene_name,
                                         HR, LCL, UCL, PVAL,
                                         out_dir,
                                         model_suffix,
                                         four_groups = TRUE) {
  ...
  if (nrow(d) < 30 || sum(d$OS_EVENT, na.rm = TRUE) < 5) {
    message("Expr:CNA KM (OS all): insufficient data (n<30 or <5 events), skipping gene: ", gene_name)
    return(NULL)
  }
  ...
  if (four_groups) {
    ...
    if (length(unique(na.omit(d$int_score))) < 2) {
      message("Expr:CNA KM (OS all): interaction score has <2 unique values, skipping gene: ", gene_name)
      return(NULL)
    }
  ...
  if (length(na.omit(unique(d$int_group))) < 2) {
    message("Expr:CNA KM (OS all): fewer than 2 interaction groups present, skipping gene: ", gene_name)
    return(NULL)
  }
  ...
  p <- ggsurvplot(
    ...
    title                = paste0(gene_name, " - Expression x CNA (OS, METABRIC)"),
    xlab                 = "Time (months)",
    ylab                 = "Probability",
    ggtheme              = theme_bw(),
    tables.theme         = theme_bw()
  ...
  message("Saved METABRIC OS expr:CNA interaction KM plot: ", out_file)
  invisible(out_file)
}

## 6e) Extra diagnostics for expr:CNA interaction genes
##     (applied to all genes with significant expr:CNA in Excel)
extra_expr_cna_diagnostics <- function(gene_name,
                                       out_dir,
                                       model_suffix,
                                       four_groups = TRUE) {
  ## Get per-gene data
  ...
  ## Basic sanity check
  if (nrow(d) < 30 || sum(d$OS_EVENT, na.rm = TRUE) < 5) {
    message("Extra expr:CNA diagnostics: insufficient data (n<30 or <5 events), skipping gene: ", gene_name)
    return(NULL)
  }
  
  ## Initialize containers for summary text
  summary_lines <- c()
  add_line <- function(...) {
  ...
  if ("int_score" %in% names(d_int) && length(unique(na.omit(d_int$int_score))) >= 3) {
    ...
    if (!is.null(cut_res_int)) {
      ...
      if (length(unique(na.omit(d_int_opt$int_group_opt))) >= 2) {
        ...
        ## surv_cutpoint cutpoint structure: column usually named "cutpoint"
        ## with rowname = variable ("int_score"). Be robust to both shapes.
  ...
  ##    - 8 groups: expr quartile x CNA
  ##    - 4 groups: expr quartiles in CNA=0 subset
  ##    - 4 groups: expr quartiles in CNA=1 subset
  expr_q <- tryCatch(
    {
  ...
  if (any(is.na(expr_q))) {
    ...
    if (length(unique(na.omit(d8$q_cna_group))) >= 2) {
      ...
      p8 <- ggsurvplot(
        ...
        title                = paste0(gene_name, " - Expr quartiles x CNA (8 groups, OS, METABRIC)"),
        xlab                 = "Time (months)",
        ylab                 = "Probability",
        ggtheme              = theme_bw(),
        tables.theme         = theme_bw()
      ...
      message("Saved METABRIC OS quartile 8-group KM plot: ", quart8_file)
    } else {
    ...
    if (nrow(d0) >= 30 &&
      ...
      p0 <- ggsurvplot(
        ...
        title                = paste0(gene_name, " - Expr quartiles (CNA = 0, OS, METABRIC)"),
        xlab                 = "Time (months)",
        ylab                 = "Probability",
        ggtheme              = theme_bw(),
        tables.theme         = theme_bw()
      ...
      message("Saved METABRIC OS quartile KM plot (CNA=0): ", quart0_file)
    } else {
    ...
    if (nrow(d1) >= 30 &&
      ...
      p1 <- ggsurvplot(
        ...
        title                = paste0(gene_name, " - Expr quartiles (CNA = 1, OS, METABRIC)"),
        xlab                 = "Time (months)",
        ylab                 = "Probability",
        ggtheme              = theme_bw(),
        tables.theme         = theme_bw()
      ...
      message("Saved METABRIC OS quartile KM plot (CNA=1): ", quart1_file)
    } else {
  ...
  if (!is.null(spline_fit)) {
    ...
    g <- ggplot(df_plot, aes(x = expr, y = HR)) +
      ...
      labs(
        title = paste0(gene_name, " - Spline Cox for expr (cna=1)"),
...
## --------------------------------------------------
## 7) Read OS Cox Excel (NEW sheet names) and loop over sheets/genes
## --------------------------------------------------
...
message("Sheets to be processed: ", paste(valid_sheets, collapse = ", "))
...
for (sh in valid_sheets) {
  ...
  for (i in seq_len(nrow(df))) {
    ...
    ## Expression KM (median + optimal) – in any sheet
    ...
    ## CNA KM (all sheets; log-rank p < 0.05)
...
message("\nAll METABRIC OS KM plots and interaction diagnostics have been saved under: ", km_dir)