...
#     pathway (ascending) + pooled HR (descending)
...
tcga_file     <- file.path(tcga_dir,     "TCGA_BRCA_OS_all_Cox_all_models_by_pathway_with_LRT.xlsx")
metabric_file <- file.path(metabric_dir, "METABRIC_OS_all_Cox_all_models_by_pathway_with_LRT.xlsx")
out_file      <- file.path(results_dir,  "METABRIC_TCGA-BRCA_OS_Pathway_Meta-Analysis.xlsx")
...
read_all_sheets <- function(path, cohort_name) {
  ...
  out <- lapply(sheets, function(sh) {
    ...
    # Make sure pathway and model exist
    if (!"pathway" %in% names(df)) stop(paste("Sheet has no 'pathway' column:", sh))
...
# - sort by pathway (asc), then relevant HR_pooled (desc)
############################################################
...
clean_filtered <- function(df, ci_col, hr_col) {
  ...
  df %>%
    ...
    arrange(pathway, desc(.data[[hr_col]]))
}
